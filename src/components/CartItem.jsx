...
const CartItem = ({ item }) => {
 const {removeFromCart, increaseAmount,decreaseAmount} = useContext(CartContext)
  ...
  return (
    <div className="flex gap-4 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500">
      <div className="w-full min-h-[150px] flex justify-center items-center gap-4">
        <Link to={`/product/${id}`}>
        ...
        <div className="w-full flex flex-col">
          <div className="flex justify-between mb-2">
            ...
            <div onClick={()=>removeFromCart(id)}>
              <IoMdClose className=" cursor-pointer text-xl text-gray-500 hover:text-red-500 transition" />{" "}
            </div>
          </div>
