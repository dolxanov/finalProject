 ...
 const {cart, clearCart, total,itemAmount} = useContext(CartContext);
  return (
    ...
    shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      ...
      gap-y-2 h-[400px] lg:h-[450px]
        border-b overflow-y-auto overflow-x-hidden'>{cart.map(item=>{
        ...
        <div className='w-full flex justify-between items-center'>
          ...
          className='cursor-pointer py-4 bg-red-500
           ...
           items-center text-xl'><FiTrash2/></div>
            ...
            <Link to={"/"} className='bg-primary flex p-4 justify-center items-center text-white w-full font-medium'> Chechkout </Link>
      </div>
    </div>
  );
}; 
